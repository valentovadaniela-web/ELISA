...
const PROTOCOLS = {
    ...
    "EVA": {
        ...
        testName: "ELISA EVA Ab (INgezim Arteritis 2.0)",
        controlPairs: { pk: ["A1","A2"], nk: ["B1","B2"] },
        formula: "cleanOD",
        validity: { maxNKcleanOD: 0.1, minPKcleanOD: 0.5 },
        interpret: { type: "cleanOD", neg: 0.1, dubLow: 0.1, dubHigh: 0.15, pos: 0.15 }
    },
    "FIP": {
    ...
    "WNV": {
        ...
        testName: "ELISA WNV IgM Capture (INgezim)",
        controlPairs: { pk: ["A1","A2"], nk: ["B1","B2"] },
        formula: "cleanOD",
...
function updateLimitsBox() {
    ...
    else if(proto.interpret.type === 'cleanOD') {
        if(currentProtocol === 'EVA') {
            interpretHtml += `<p><span style="color:var(--success)">■</span> NEGATÍVNA: < 0,1</p><p><span style="color:var(--warning)">■</span> DUBIÓZNA: 0,1 - 0,15</p><p><span style="color:var(--danger)">■</span> POZITÍVNA: > 0,15</p>`;
        } else {
            interpretHtml += `<p><span style="color:var(--success)">■</span> NEGATÍVNA: ≤ ${proto.interpret.neg}</p><p><span style="color:var(--warning)">■</span> DUBIÓZNA: ${proto.interpret.dubLow} - ${proto.interpret.dubHigh}</p><p><span style="color:var(--danger)">■</span> POZITÍVNA: ≥ ${proto.interpret.pos}</p>`;
        }
    }
...
function getSamplePairs() {
    const proto = PROTOCOLS[currentProtocol];
    let controlSet = new Set();
    if(proto.controlPairs) { 
        if(proto.controlPairs.pk) proto.controlPairs.pk.forEach(w=>controlSet.add(w)); 
        if(proto.controlPairs.nk) proto.controlPairs.nk.forEach(w=>controlSet.add(w)); 
    }
    const pairs = [];
    const isCleanOD = (proto.formula === "cleanOD");
    
    for(let r of ROWS) {
        for(let i=1; i<=12; i+=2) { 
            let well1 = `${r}${i}`;     // stĺpec 1,3,5,7,9,11
            let well2 = `${r}${i+1}`;   // stĺpec 2,4,6,8,10,12
            
            if(!controlSet.has(well1) && !controlSet.has(well2)) {
                if(isCleanOD) {
                    // Pre EVA/WNV: well1 = Ag+, well2 = Ag- (podľa Excel tabuľky)
                    // clean = (Ag+) - (Ag-)
                    pairs.push({ minus: well2, plus: well1 });
                } else {
                    // Pre ostatné protokoly podľa pôvodnej logiky
                    pairs.push({ minus: well1, plus: well2 });
                }
            }
        }
    }
...
function vypocitaj(save = true) {
    ...
    if(proto.formula === "cleanOD") { // EVA, WNV
        ...
        let idx=0;
        for(let p of pairs) {
            const odMinus=getV(p.minus), odPlus=getV(p.plus);
            if(odMinus===0 && odPlus===0 && document.getElementById(p.minus).value.trim()==="") continue;
            const clean = odPlus - odMinus;  // Ag+ - Ag-
            let verdict="", cls="";
            
            if(currentProtocol === "EVA") {
                // EVA podľa Excelu: NEG < 0.1, DUB 0.1-0.15, POZ > 0.15
                if(clean < 0.1) { verdict="Negatívna"; cls="badge-neg"; }
                else if(clean > 0.15) { verdict="Pozitívna"; cls="badge-pos"; }
                else { verdict="Dubiózna"; cls="badge-dub"; }
            } else {
                // WNV podľa Excelu: NEG ≤ 0.30, DUB 0.30-0.35, POZ ≥ 0.35
            ...
            const vzid = (idx<idArr.length)?idArr[idx]:`Vzorka ${p.plus}/${p.minus}`;
            // Zobrazenie: Ag+ / Ag- (plus / minus)
            tbody.innerHTML += `<tr><td>${p.plus}/${p.minus}</td><td>${vzid}</td><td>${odPlus.toFixed(3)} / ${odMinus.toFixed(3)}</td><td>${clean.toFixed(3)}</td><td><span class="badge ${cls}">${verdict}</span></td></tr>`;
            idx++;
        }
        ...
        for(let w of wells) {
            ...
            tbody.innerHTML += `｜｜DSML｜｜<td>${w}</td><td>${vzid}</td><td>${od.toFixed(3)}</td><td>${blok.toFixed(1)}%</td><td><span class="badge ${cls}">${verdict}</span></td></tr>`;
            idx++;
        }
        ...
        if(currentBvdType==='serum') {
            ...
            for(let w of wells) {
                ...
                const vzid = (idx<idArr.length)?idArr[idx]:`Vzorka ${w}`;
                tbody.innerHTML += `<tr><td>${w}</td><td>${vzid}<tr><td>${od.toFixed(3)}</td><td>negCutoff=${negCutoff.toFixed(3)}<br>posCutoff=${posCutoff.toFixed(3)}</td><td><span class="badge ${cls}">${verdict}</span></td></tr>`;
                idx++;
            }
        } else {
        ...
        for(let w of wells) {
            ...
            const vzid = (idx<idArr.length)?idArr[idx]:`Vzorka ${w}`;
            tbody.innerHTML += `<tr><td>${w}</td><td>${vzid}<tr><td>${od.toFixed(3)}</td><td>S/N = ${sn.toFixed(1)}%</td><td><span class="badge ${cls}">${verdict}</span></td></tr>`;
            idx++;
        }
